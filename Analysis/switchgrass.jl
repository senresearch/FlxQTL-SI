...
#genetic kinship
@time K=kinshipLoco(FlxQTL.kinshipLin,XX,4);
@time T, Λ=K2eig(K,true);

#1D marker scan
@time  lod1,B1,est01 =geneScan(4,T,Λ,Y3,XX,true;H0_up=true,df_prior=Int64(1.5m));

writedlm("../Result/switchgrass_fl50_36site_yr_1d_lods.txt",lod1)
...
# permutation test
@time maxLODs, h1ppar ,cutoff = permutationTest(1000,4,K,Y3,XX;df_prior=Int64(1.5m));
